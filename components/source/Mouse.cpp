...
void Mouse::rightMotion(int x, int y)
{
    ...
    else
    {
        ...
        {
            spot_light = dynamic_cast<SpotLight*>(Globals::world->get_curr_object());
            
            if (spot_light != nullptr)
            {
                light = spot_light->get_light_pointer();
                cutoff = light->get_property(Light::SPOT_CUTOFF);
                ...
                if (location[1] < old_location[1])
                    light->set_property(Light::SPOT_CUTOFF, cutoff - 1.0);
                else
                    light->set_property(Light::SPOT_CUTOFF, cutoff + 1.0);
            }
        }
